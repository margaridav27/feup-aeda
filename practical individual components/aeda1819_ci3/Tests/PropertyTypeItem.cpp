...
bool PropertyTypeItem::operator<(const PropertyTypeItem &pti1) const {
    ...
    else if (address == pti1.getAddress() && postalCode < pti1.getPostalCode()) return true;
    else if (address == pti1.getAddress() && postalCode == pti1.getPostalCode() && typology < pti1.getTypology()) return true;
    else if (address == pti1.getAddress() && postalCode == pti1.getPostalCode() && typology == pti1.getTypology() && maxPrice < pti1.getMaxPrice()) return true;
    return false;
}

bool PropertyTypeItem::operator==(const PropertyTypeItem &pti1) const {
    if (address == pti1.getAddress() && postalCode == pti1.getPostalCode() && typology == pti1.getTypology()) return true;
    else return false;
}