...
int AuthorRecord::addAvailablePaint() {
    return ++availablePaints;
}
...
//TODO
bool PaintCatalogItem::operator<(const PaintCatalogItem& pci) const {
    if (getYear() > pci.getYear()) return true;
    else if (getYear() == pci.getYear() && getAuthor() < pci.getAuthor()) return true;
    else if (getYear() == pci.getYear() && getAuthor() == pci.getAuthor() && getTitle() < pci.getTitle()) return true;
    return false;
}
...
bool ExhibitionItem::operator<(const ExhibitionItem& pci) const {
    if (getYear() > pci.getYear()) return true;
    else if (getYear() == pci.getYear() && getPrice() < pci.getPrice()) return true;
    return false;
}
