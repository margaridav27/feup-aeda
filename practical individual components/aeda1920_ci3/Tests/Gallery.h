...
struct AuthorRecordHash {
    ...
    int operator()(const AuthorRecord& ar) const {
        int hash = 0;
        for (unsigned i = 0; i < ar.getAuthor().length(); i++) {
            hash = 37 * hash + ar.getAuthor()[i];
        }
        return hash;
    }
...
public:
    ...
    vector<Paint*> nBestExhibition(int n, int maxPerYear); //escolhe as N mais caras pinturas, dos anos mais antigos, para realizar uma exposicao (maximo de NPerYear pinturas de um mesmo ano)
    int itemExhibitionOrder(string a, string t); // verifica qual a ordem de exibicao de uma pintura, mantentdo a ordem origial dos elementos; retorna 0, se nao estiver na fila
};
