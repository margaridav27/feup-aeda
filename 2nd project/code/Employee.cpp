...
ServiceProvider::ServiceProvider(string name, unsigned int nif, unsigned int age, double perHourSalary,
                                 unsigned int numMaintenances) : Employee(name, nif, age) {
...
bool ServiceProviderType::operator<(const ServiceProviderType toCmp) const {
    ...
    else if (sp->getNumMaintenancesDone() == toCmp.getServiceProvider()->getNumMaintenancesDone() &&
             sp->getEvaluation() < toCmp.getServiceProvider()->getEvaluation())
        return true;
    return false;
}
...
ContractedEmployee::ContractedEmployee(string name, unsigned int nif, unsigned int age, double perMonthSalary)
        : Employee(name, nif, age) {
...
bool ContractedEmployeeType::operator<(const ContractedEmployeeType toCmp) const {
    ...
    else if (ce->getEvaluation() == toCmp.getContractedEmployee()->getEvaluation() &&
             ce->getNumClientsAssociated() > toCmp.getContractedEmployee()->getNumClientsAssociated())
        return true;
    return false;
}
