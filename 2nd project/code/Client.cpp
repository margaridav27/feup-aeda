...
bool Client::operator==(Client *comp) {
    return (name == comp->name && nif == comp->nif);
}
...
Entrepreneur::Entrepreneur(string name, unsigned int nif, string date, unsigned int employeeNif) : Client(name, nif,
                                                                                                          date) {
...
string Entrepreneur::show() {
    stringstream info;
    info << "NIF: " << nif << " | NAME: " << name << " | NUMBER OF LIFTS: " << lifts.size() << " | EMPLOYEE'S NIF: "
         << employeeNif;
    return info.str();
}
