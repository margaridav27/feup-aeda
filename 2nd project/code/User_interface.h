...
public:
    ...
    /**
     * verifies if the date d given by parameter is after of before today
     * @param d is a date given by input
     * @return true if it is possible to schedule maintenance, false otherwise
     */
    bool ValidDateToMaintenance(Date d);

    /**
 * verifies if the time t given by parameter is after of before now
 * @param t is a time given by input
 * @return true if it is possible to schedule maintenance, false otherwise
 */
    bool ValidTimeToMaintenance(Time t);
    ...
    /**
     * Function where the uses schedules a maintenance
     * @param client that is scheduling
     */
    void scheduleMaintenance(Client *client);
